...
export default function TechnologyScreen() {
  ...
  const loadNews = async (useCache = true) => {
    try {
      ...
      if (useCache) {
        ...
        const cachedNews = await StorageService.getCachedNews(NEWS_CATEGORIES.TECHNOLOGY);
  ...
  const fetchFreshNews = async () => {
    try {
      const response = await NewsAPI.fetchNews(NEWS_CATEGORIES.TECHNOLOGY);
      
      if (response.articles && response.articles.length > 0) {
        setArticles(response.articles);
        
        // Cache the articles
        await StorageService.cacheNews(NEWS_CATEGORIES.TECHNOLOGY, response.articles);
        
        setError(null);
      } else {
        setError('No technology articles found');
      }
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      <NewsList
        ...
        emptyMessage="No technology news available at the moment. Pull down to refresh."
      />
    </View>
  );
}