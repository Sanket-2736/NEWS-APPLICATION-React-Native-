...
export default function BusinessScreen() {
  ...
  const loadNews = async (useCache = true) => {
    try {
      ...
      if (useCache) {
        ...
        const cachedNews = await StorageService.getCachedNews(NEWS_CATEGORIES.BUSINESS);
  ...
  const fetchFreshNews = async () => {
    try {
      const response = await NewsAPI.fetchNews(NEWS_CATEGORIES.BUSINESS);
      
      if (response.articles && response.articles.length > 0) {
        setArticles(response.articles);
        
        // Cache the articles
        await StorageService.cacheNews(NEWS_CATEGORIES.BUSINESS, response.articles);
        
        setError(null);
      } else {
        setError('No business articles found');
      }
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      <NewsList
        ...
        emptyMessage="No business news available at the moment. Pull down to refresh."
      />
    </View>
  );
}